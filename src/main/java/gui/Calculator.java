	...
	public void handle(ActionEvent event) {
		...
		if ("0123456789".contains(value)) {
	        ...
	        if (!number1.isEmpty() && !number2.isEmpty() && operator != null) {
	            double result = calculate(Double.parseDouble(number1), Double.parseDouble(number2), operator);
	            if (err) {
	            	...
	            	if (result != (int) result) displayText.setText(String.valueOf(result));
	            	else displayText.setText(String.valueOf((int) result));
